            ...
            <div className={`max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-6 transition-all ${data?.user.age != null && data?.user.verified == false ? "blur-md pointer-events-none" : ""}`}>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <div className="p-4 rounded-lg">
                        ...
                        <select
                            name="gender"
                            value={data?.user.gender || formData.gender}
                            disabled={data?.user?.verified}
                            onChange={handleChange}
                            className="w-full border-2 border-indigo-100 rounded-md px-3 py-2 focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 outline-none transition-all"
                        >
                    ...
                    <div className="p-4 rounded-lg">
                        ...
                        <input
                            type="tel"
                            name="phone"
                            value={data?.user.phone || formData.phone}
                            disabled={data?.user?.verified}
                            onChange={handleChange}
                            className="w-full border-2 border-indigo-100 rounded-md px-3 py-2 focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 outline-none transition-all"
                        />
                    ...
                    <div className="p-4 rounded-lg">
                        ...
                        <input
                            ...
                            name="address"
                            disabled={data?.user?.verified}
                            value={data?.user.address || formData.address}
                            onChange={handleChange}
                            className="w-full border-2 border-indigo-100 rounded-md px-3 py-2 focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 outline-none transition-all"
                        />
                    </div>
                    {data?.user?.verified && (<div className='w-full flex justify-center'>
                        <Image src={data.user.adhar || ''} alt="adhar" width={200} height={200} />
                    </div>)}
                    {!data?.user?.verified && (
                    ...
                    <div className="md:col-span-2 p-4">
                        ...
                        >
                            {data?.user?.verified ? 'Verified' : 'Submit'} Application
                        </button>
                    </div>
                </form>
