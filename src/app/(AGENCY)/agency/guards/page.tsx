...
const page = () => {
  const [guardData, setGuardData] = useState<Array<{
    id: number;
    name: string;
    email: string;
    phone: string;
            ...
            <table className="min-w-full table-auto">
              ...
              <tbody>
                {guardData.map((guard: any) => (
                  <tr key={guard.id} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                   
                              <td  className="px-6 py-4 text-sm text-gray-700 cursor-pointer">{guard.id}</td>
                              <td  className="px-6 py-4 text-sm text-gray-700 font-medium cursor-pointer">{guard.name}</td>
                              <td  className="px-6 py-4 text-sm text-gray-700 cursor-pointer">{guard.email}</td>
                              <td  className="px-6 py-4 text-sm text-gray-700 cursor-pointer">{guard.phone}</td>
                              <td  className="px-6 py-4 text-sm text-gray-700 cursor-pointer">{guard.age}</td>
                              <td  className="px-6 py-4 text-sm text-gray-700 cursor-pointer">{guard.gender}</td>
                              <td  className="px-6 py-4 text-sm text-gray-700 cursor-pointer">{guard.address}</td>
                  </tr>
                ))}
              </tbody>
            </table>
